...
export default function TelefonosPlazosDesktop({ datosTelefonos }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      let file = "/reports/informe_telefonos.mrt";
      ...
      const dataSet = new window.Stimulsoft.System.Data.DataSet("Telefonos");
      ...
      /*  setTimeout(() => {
         try {
           ...
           if (elements.length > 0) {
             Object.assign(elements[0].style, {
           ...
           const menus = document.getElementsByClassName('stiJsViewerMenu');
           for (let i = 0; i < menus.length; i++) {
             Object.assign(menus[i].style, {
               zIndex: 11,
               position: 'relative',
               width: '220px',
               height: 'auto',
           ...
           const viewerDiv = document.getElementById('StiViewer');
           if (viewerDiv) viewerDiv.style.height = '5%';
 
           const panel = document.getElementById('StiViewerReportPanel');
           if (panel) panel.style.width = '100%';
 
           console.log("Reporte renderizado correctamente");
         } catch (styleError) {
           console.warn("Error aplicando estilos al visor:", styleError);
         }
       }, 300); */
