...
export default function TelefonosDesktop({ datosTelefonos }) {
  ...
  useEffect(() => {
    if (!datosTelefonos || datosTelefonos.length < 2) {
    ...
    requestAnimationFrame(movePanelToRight);
     */
    let json = JSON.stringify({ Telefonos: datosTelefonos }, null, 2);
    ...
    fetch("/stireport/Localization/es.xml").then(r => console.log(r.status));


    // --- 1) Cargar la localización (ANTES de crear el viewer)
    ...
    if (datosTelefonos.anterior) file = "/reports/comparativa_agente2.mrt";
    ...
    let dataSet = new window.Stimulsoft.System.Data.DataSet("Telefonos");
  ...
  const prepararDatos = (datosTelefonos) => {
    const empresaPrincipal = datosTelefonos[0];
    const empresasComparadas = datosTelefonos.slice(1);
    ...
    /* const jsonString = JSON.stringify({ Telefonos: datosReporte, condicion: [obj] }, null, 2);
    ...
    return { Telefonos: datosReporte, condicion: [obj] };
  };
